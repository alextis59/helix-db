...
export const validateExamplePolicy = (candidate = readJson(policyPath)) => {
  ...
  const nativeLock = readText(candidate.native.lockfile);
  assert(
    nativeLock.startsWith('# This file is automatically @generated by Cargo.\n'),
    'native lock header',
  );
  assert(
    !/^source = /m.test(nativeLock) && !/^checksum = /m.test(nativeLock),
    'native example external dependency',
  );
  same(
    [...nativeLock.matchAll(/^name = "([^"]+)"$/gm)].map((match) => match[1]),
    [
      'helix-columnar',
      'helix-core',
      'helix-doc',
