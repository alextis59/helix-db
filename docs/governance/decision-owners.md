...
| [HDoc v1 physical baseline](../adr/0012-use-bounded-little-endian-hdoc-v1.md): checksum, compression, endianness, alignment, offsets, canonical hash, path dictionary, and extensions | Storage architecture owner | Query semantics, runtime, security | ADR 0012 accepted; exact HDoc subordinate layouts complete through `P03-007`; encoder, validating reader, values, lookup, tagged conversion, path-dictionary format, and registration/resolution/recovery/pinning implemented at `P03-008`–`P03-014`; immutable fixtures remain | `P03-001`–`P03-014` |
