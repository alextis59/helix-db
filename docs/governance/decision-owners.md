...
| [HDoc v1 physical baseline](../adr/0012-use-bounded-little-endian-hdoc-v1.md): checksum, compression, endianness, alignment, offsets, canonical hash, path dictionary, and extensions | Storage architecture owner | Query semantics, runtime, security | ADR 0012 accepted; layouts through `P03-007`; codec, values, lookup, conversion, dictionary format/lifecycle, negotiation, immutable fixtures, cross-language parity, deterministic hardening, pinned fuzz/browser replay, and representative raw benchmarks implemented at `P03-008`–`P03-020`; P03-021 interpretation and gate review remain | `P03-001`–`P03-021` |
