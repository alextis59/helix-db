...
This contract proves that the portable foundation crate emits two different valid WebAssembly artifact forms and that the browser form survives the complete Rust-to-Vite-to-real-browser path. It does not define the production host ABI, implement database behavior, expose a user-facing example, or establish a supported release platform.
...
The committed app under `tests/browser/smoke-app/` is an internal toolchain fixture, not a product example. Vite builds it from a fixed root into ignored `dist/browser` output with:
...
`test:browser` remains the stable binary-free inventory command and now requires exactly three project-expanded tests in one file. The full browser suite stays reserved until `P02-016` adds the first user-facing example and broadens it beyond the internal toolchain smoke. CI uses the explicit `ci:browser-smoke` command because browser downloads are deliberate network/platform provisioning, not an npm lifecycle side effect.
...
This step does not prove database functionality, JavaScript bindings, an ergonomic SDK, component execution in a WASI host, async/capability ABI correctness, storage/GPU capability detection, OPFS/IndexedDB fallback, branded Chrome/Edge/Safari support, packaging, offline installation, or release reproducibility. `P02-016` owns minimal user-facing examples, `P04-*` owns the component ABI/host, `P11-*` owns real browser lifecycle/capability/storage behavior, and `P16-*`/`P24-*` own packaged support claims.
