...
| Portable Rust | `wasm32-unknown-unknown`, `wasm32-wasip2` on Linux x64 | Strict Clippy plus real core-module/component builds; the component lane checks immutable ABIs 1.0–4.0 and current explicit-copy ABI 5.0/reference model with their canaries, then validates/classifies the still-unbound WASIp2 artifact |
